// Collection
import Nature from "../assets/images/nature.jpeg";
import Wedding from "../assets/images/wedding.jpeg";
import Party from "../assets/images/party.jpeg";
import Business from "../assets/images/business.jpeg";
import Fashion from "../assets/images/fashion.jpeg";
import Family from "../assets/images/family.jpeg";
// Gallery
import Img1 from "../assets/images/img1.jpeg";
import Img2 from "../assets/images/img2.jpeg";
import Img3 from "../assets/images/img3.jpeg";
import Img4 from "../assets/images/img4.jpeg";
import Img5 from "../assets/images/img5.jpeg";
import Img6 from "../assets/images/img6.jpeg";
import Img7 from "../assets/images/img7.jpeg";
import Img8 from "../assets/images/img8.jpeg";
import Img9 from "../assets/images/img9.jpeg";
import Img10 from "../assets/images/img10.jpeg";
import Img11 from "../assets/images/img11.jpeg";
import Img12 from "../assets/images/img12.jpeg";
import Img13 from "../assets/images/img13.jpeg";
import Img14 from "../assets/images/img14.jpeg";
import Img15 from "../assets/images/img15.jpeg";
import Img16 from "../assets/images/img16.jpeg";
// Customers
import Customer1 from "../assets/images/customer1.jpeg";
import Customer2 from "../assets/images/customer2.jpeg";
import Customer3 from "../assets/images/customer3.jpeg";

export const navbarText = {
  title_1: "photo",
  title_2: "X",
  navItems: [
    "Home",
    "Mission",
    "Collection",
    "Gallery",
    "Customers",
    "Pricing",
    "Contact",
  ],
};

export const bannerText = {
  initial_1: "Welcome to ",
  initial_2: "photo",
  initial_3: "X",
  content: "Lorem ipsum dolor sit amet consectetur adipisicing.",
};

export const missionText = {
  main: {
    title: "Our Mission",
    content:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium inventore, sint quisquam fugiat pariatur culpa officia. Eveniet omnis quia tempora.",
  },
  secondary: [
    {
      title: "Creativity",
      content:
        "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea hic perferendis dolor vitae sed, quae officiis exercitationem quaerat? Fugit excepturi exercitationem inventore, repellendus architecto illum!",
      class: "fas fa-cogs fa-5x text-danger mb-4",
    },
    {
      title: "Quality",
      content:
        "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea hic perferendis dolor vitae sed, quae officiis exercitationem quaerat? Fugit excepturi exercitationem inventore, repellendus architecto illum!",
      class: "far fa-thumbs-up fa-5x text-danger mb-4",
    },
    {
      title: "Experience",
      content:
        "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea hic perferendis dolor vitae sed, quae officiis exercitationem quaerat? Fugit excepturi exercitationem inventore, repellendus architecto illum!",
      class: "far fa-handshake fa-5x text-danger mb-4",
    },
  ],
  objective: {
    title: "We know what we do",
    content:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Eveniet tempora itaque obcaecati voluptas? Perferendis voluptate accusantium eum sit deleniti harum, assumenda vitae! Cupiditate eos iusto ab rerum, voluptatum minima sed?",
  },
};

export const collectionText = {
  main: {
    title: "Collection",
    content:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium inventore, sint quisquam fugiat pariatur culpa officia. Eveniet omnis quia tempora.",
  },
  collection: [
    { title: "Nature Photography", image: Nature },
    { title: "Wedding Photography", image: Wedding },
    { title: "Party Photography", image: Party },
    { title: "Business Photography", image: Business },
    { title: "Fashion Photography", image: Fashion },
    { title: "Family Photography", image: Family },
  ],
};

export const galleryText = {
  main: {
    title: "Gallery",
    content:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium inventore, sint quisquam fugiat pariatur culpa officia. Eveniet omnis quia tempora.",
  },
  images: [
    Img1,
    Img2,
    Img3,
    Img4,
    Img5,
    Img6,
    Img7,
    Img8,
    Img9,
    Img10,
    Img11,
    Img12,
    Img13,
    Img14,
    Img15,
    Img16,
  ],
};

export const customerText = {
  main: {
    title: "Happy Customers",
    content:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium inventore, sint quisquam fugiat pariatur culpa officia. Eveniet omnis quia tempora.",
  },
  indicator: ["0", "1", "2"],
  users: [
    {
      description:
        "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Eum earum fuga natus veritatis minima voluptatibus? Quae consequuntur beatae repudiandae aut!",
      name: "Monica",
      stars: 5,
      image: Customer1,
    },
    {
      description:
        "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Eum earum fuga natus veritatis minima voluptatibus? Quae consequuntur beatae repudiandae aut!",
      name: "Jonatan",
      stars: 3,
      image: Customer2,
    },
    {
      description:
        "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Eum earum fuga natus veritatis minima voluptatibus? Quae consequuntur beatae repudiandae aut!",
      name: "Maria",
      stars: 4,
      image: Customer3,
    },
  ],
};

export const pricingText = {
  main: {
    title: "Join Us",
    content:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium inventore, sint quisquam fugiat pariatur culpa officia. Eveniet omnis quia tempora.",
  },
  signup: "sign-up!",
  prices: [
    {
      title: "Unlimited Access",
      value: "499",
      list: [
        "Photoshop",
        "After Effects",
        "Graphic Design",
        "Video Montage",
        "Clip Making",
      ],
    },
    {
      title: "Annual Membership",
      value: "199",
      list: ["Photoshop", "After Effects", "Graphic Design", "Video Montage"],
    },
    {
      title: ">Monthly Membership",
      value: "19",
      list: [
        "Photoshop",
        "After Effects",
        "Graphic Design",
        "Video Montage",
        "Clip Making",
      ],
    },
  ],
  gratitude: "Thanks for being with us!",
};

export const contactText = {
  title: "Get In Touch",
};

export const footerText = {
  main: {
    title: "About Us",
    content:
      "Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ipsam nobis dicta molestiae id laboriosam natus repudiandae, ducimus illum veritatis perspiciatis possimus, at facere debitis accusantium?",
  },
  visit: "Visit Us",
  links: [
    "Home",
    "Mission",
    "Collection",
    "Gallery",
    "Customers",
    "Pricing",
    "Contact",
  ],
  help: "Need Help?",
  services: ["Customer Service", "Online Chat", "Support", "photox@email.com"],
  connected: {
    title: "Stay Connected",
    content:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Itaque accusamus dolores iste praesentium assumenda est quia accusantium corrupti ipsam inventore.",
  },
  copyright: new Date().getFullYear() + " Copyright, All Rights Reserved",
};
